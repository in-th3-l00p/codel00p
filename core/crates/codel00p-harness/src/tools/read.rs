//! Read-only workspace inspection tools: `list_files`, `read_file`, and
//! `search_text`.
//!
//! These are the model's eyes on the project. They share the workspace walk
//! (ignore-aware, resilient to unreadable subdirectories) and the `verbosity` /
//! pagination conventions defined in the parent [`super`] module, so the result
//! shapes stay consistent across read tools.
...
use super::{Tool, optional_string, parse_verbosity, required_string, verbosity_schema};
