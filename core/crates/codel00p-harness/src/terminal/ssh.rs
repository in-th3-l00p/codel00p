...
fn build_base_ssh_options(config: &SshConfig, control_socket: Option<&Path>) -> Vec<String> {
...
fn build_exec_command(remote_cwd: &str, spec: &CommandSpec) -> String {
...
fn wrap_with_timeout(inner: &str, secs: u64) -> String {
...
fn build_background_command(remote_cwd: &str, spec: &CommandSpec, marker: &str) -> String {
...
fn build_kill_command(marker: &str) -> String {
...
fn build_read_file_command(workspace: &Path, rpath: &str) -> String {
...
fn build_write_file_command(rpath: &str) -> String {
    format!(
...
fn build_delete_file_command(workspace: &Path, rpath: &str) -> String {
    let ws = workspace_str(workspace);
    format!(
...
fn build_mkdir_command(rpath: &str) -> String {
...
fn build_metadata_command(workspace: &Path, rpath: &str) -> String {
    let ws = workspace_str(workspace);
...
fn build_read_dir_command(workspace: &Path, rpath: &str) -> String {
    let ws = workspace_str(workspace);
    format!(
...
fn build_walk_command(rpath: &str, include_ignored: bool) -> String {
    if include_ignored {
...
fn boundary_glob(ws: &str) -> String {
...
fn remote_parent(rpath: &str) -> String {
    match rpath.rfind('/') {
        Some(0) => "/".to_string(),
...
fn workspace_str(workspace: &Path) -> String {
...
fn remote_workspace_path(remote_workspace: &Path, rel: &Path) -> Result<String, HarnessError> {
...
/// [`local_fs`](super::local_fs) so the backend is self-contained.
...
fn reject_absolute_for_write(rel: &Path) -> Result<(), HarnessError> {
    reject_unsafe_components(rel)?;
    if rel.is_absolute() {
        return Err(HarnessError::WorkspaceEscape {
...
fn parse_metadata(stdout: &str) -> FileKind {
...
fn parse_read_dir(stdout: &str) -> Vec<DirEntry> {
    stdout
        .lines()
        .filter_map(|line| {
...
fn parse_walk<'a>(stdout: &'a str, workspace: &'a str) -> impl Iterator<Item = String> + 'a {
