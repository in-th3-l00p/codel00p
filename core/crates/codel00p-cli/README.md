...
- `memory quality` prints active candidate or approved memories with low
  ...
  `content`; add `--kind <kind>` or `--sensitivity normal|sensitive` to focus
  one review lane and `--json` for quality record objects. The MCP
  `memory_quality` tool returns the same review queue.
