...
fn plugins_list_shows_builtin_catalog_disabled_by_default() {
    ...
    let output = run(home.path(), &["plugins", "list"]);
...
fn plugins_enable_disable_round_trip() {
    ...
    let enable = run(home.path(), &["plugins", "enable", "system-info"]);
    ...
    let listed = run(home.path(), &["plugins", "list"]);
...
fn plugins_enable_rejects_unknown_plugin() {
    let home = tempdir().expect("tempdir");
    let output = run(home.path(), &["plugins", "enable", "nope"]);
...
fn plugins_enable_is_idempotent() {
    let home = tempdir().expect("tempdir");
    assert!(
        run(home.path(), &["plugins", "enable", "system-info"])
            .status
            .success()
    );
    let again = run(home.path(), &["plugins", "enable", "system-info"]);
