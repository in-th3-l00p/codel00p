...
/**
 * Completes CLI sign-in by redirecting the browser to the CLI's localhost
 * loopback (carrying the session token). Shown briefly before the redirect, with
 * a manual fallback link and an error state.
 */
  ...
  return (
    <main className="relative grid min-h-screen place-items-center px-6">
      ...
      <div className="rise w-full max-w-md rounded-2xl border border-border bg-card/40 p-8 text-center backdrop-blur-sm">
        ...
        <p className="label text-muted-foreground/70">codel00p · cli</p>
        ...
        {error ? (
          <>
            ...
            <p className="mt-4 text-xs text-muted-foreground">
              Return to your terminal and run <code>codel00p login</code> again.
            </p>
          </>
        ) : (
          <>
            ...
            <p className="mt-4 text-sm leading-relaxed text-muted-foreground">
              {redirected
                ? "Returning you to the terminal — you can close this tab."
                : "Handing your session back to the codel00p CLI…"}
            </p>
            {loopback ? (
              <a
                href={loopback}
