...
- [x] A **curator** pass (codel00p already has staleness/quality scoring): opt-in consolidation of near-duplicate learned skills/memories per agent (shingle similarity from #107), archive-not-delete, behind review — fights skill/memory sprawl. Shipped as `codel00p memory curate` + the near-duplicate pass of `codel00p skills curate`, gated by the default-OFF `agent.behavior.curator` toggle. Detection reuses the new dependency-light `codel00p-textsim` crate (shingle/Jaccard, no LLM); consolidation is deterministic keep-best (memory: highest quality score; skill: most-used) + archive-the-rest, reversible via the existing review/archive paths. See `docs/superpowers/plans/2026-06-25-agent-curator.md`. The full-screen review dialogs surface curator findings inline (rows annotated `~dup of X (n%)` when the curator is on) with a `c` consolidate key — skills route through the existing disable-confirm/archive path, memory emits a reversible Archive — so a reviewer can act on near-duplicates without leaving the TUI.
...
- [x] **Routing/orchestration**: use each agent's `description` to route delegated tasks to the right specialist agent (pairs with sub-agent fan-out in #12 / kanban). Shipped as a deterministic, offline BM25 routing primitive (`agent/routing.rs`: `rank_agents` / `best_match` over each agent's name + description + persona) exposed via `agent route <task> [--json] [--limit N]`. Explainable and side-effect-free — it returns ranked matches + a single best pick for a caller to act on. Not auto-wired into live concurrent delegation: the spawner swaps a process-global `CODEL00P_HOME`, so per-agent home swapping across parallel children is a concurrency hazard left for the spawner rework; the primitive + JSON seam are the safe, testable foundation.
- [x] **Pluggable external memory provider seam** (Honcho-style running user-model) behind the `MemoryRanker`/provider seam from #107 — optional, additive, governance-gated (sends content out). Shipped as an object-safe `RankingProvider` trait the store holds behind `Arc<dyn RankingProvider>` (the algorithmic `MemoryRanker::rank<K>` is generic ⇒ not object-safe, so the injection trait is separate); default is the offline `Bm25RankingProvider`, injected via `with_ranker`. The CLI's `ExternalRanker` POSTs query + candidate content to a configured service and falls back to BM25 on any failure. Governance-gated and fail-closed via `[memory]` config: external ranking is used only with `ranker = "external"` + a non-empty `external_url` + `allow_external_ranking = true`, so memory content never leaves the host unless the operator opts in on all three.
