...
but they do not appear in this cleanup queue. Reviewers can filter the queue by
memory kind, sensitivity, or tag. The CLI command is `memory quality`; MCP
exposes the same data through `memory_quality`.
