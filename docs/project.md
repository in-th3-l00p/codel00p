# Project Description

codel00p is an agentic coding platform for teams that want AI agents to
understand their real projects over time.

The product is not just a coding agent. It is a system for building, reviewing,
retrieving, and sharing project knowledge across human developers, cloud
workspaces, local runtimes, and agent sessions.

In product terms, codel00p is aiming for Claude Code-grade coding-agent
execution, Hermes-grade provider breadth, and a team control plane for project
memory, AI usage, providers, permissions, and audit history. See
[Product Roadmap](product-roadmap.md) for the staged path.

## Core thesis

The strongest long-term advantage of an agentic coding system is not only model
quality. It is the quality of the surrounding harness, tools, context, memory,
and team workflow.

codel00p should make those parts explicit:

- the harness runs agent work;
- the memory layer preserves useful project knowledge;
- the interfaces help developers supervise work and curate memory;
- the cloud layer makes shared knowledge, providers, and governance powerful;
- the final product brings these pieces together into one polished experience.

## Product principles

### Knowledge-first

Project knowledge is the product's core asset. The platform should optimize for
high-quality reviewed memory that helps teams and agents understand the codebase
better over time.

### Cloud-capable

Cloud workspaces should be a first-class path because team memory, provider
governance, shared access, audit history, and cross-project visibility are more
powerful when coordinated centrally.

### Provider-flexible

Inference should be configurable. A user may use local credentials, an
organization-provided provider, or a future self-hosted/local model endpoint.

### Modular

Each subproject should have a clear responsibility and a stable interface. The
final `codel00p` product should integrate modules that are independently useful.

### Open-source by default

The project should be understandable, hackable, and useful without closed
infrastructure. Cloud features can provide the strongest team experience, but
the open-source system should remain inspectable and adaptable.

## Final product

The polished `codel00p` product should provide:

- an agent harness for repository work;
- a CLI for fast developer workflows;
- a desktop app for session supervision and memory review;
- a cloud platform for organization memory, teams, and providers;
- provider routing for local and organization-managed inference;
- project memory that improves over time.

The first practical milestone is smaller: prove that reviewed project memory can
make agent sessions better.
